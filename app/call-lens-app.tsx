...
import { audioAnalysisSettingsSchema, type AudioMetrics, type DiarizedTranscriptionResult, type SpeakerMappingEntry } from "./lib/audio-domain";
import { transcriptionWorkspaceSettingsSchema } from "./lib/arabic-intelligence.ts";
...
function Analyze() {
  ...
  const { state, settings, updateState, updateMetadata, updateSettings, setAnalysisResult } = useCallLensState();
  ...
  return <section className="content narrow"><div className="page-intro"><span className="eyebrow accent">NEW LIVE ANALYSIS</span><h1>Turn a conversation into clear coaching.</h1><p>Paste a transcript or securely upload a recording. No AI call is attempted until a server-side key is configured.</p></div>
    ...
    <div className="panel setup-card"><div className="step-title"><span>2</span><div><h2>Analysis setup</h2><p>Help CallLens interpret the conversation</p></div></div><label>AGENT NAME</label><input className="full-input" value={state.metadata.agentName} onChange={(event)=>updateMetadata({agentName:event.target.value})}/><label>LANGUAGE</label><div className="segmented">{(["Auto Detect","English","Arabic"] as const).map((value)=><button key={value} onClick={()=>updateMetadata({selectedLanguage:value})} className={state.metadata.selectedLanguage===value?"selected":""}>{value === "Arabic" ? "العربية" : value}</button>)}</div><label className="transcription-mode-label">TRANSCRIPTION MODE</label><div className="segmented transcription-mode" aria-describedby="transcription-mode-help">{(["standard","arabic_intelligence"] as const).map((value)=><button key={value} onClick={()=>updateSettings({...settings,transcription:{...settings.transcription,mode:value}})} className={settings.transcription.mode===value?"selected":""}>{value === "standard" ? "Standard" : "Arabic Intelligence"}</button>)}</div><p id="transcription-mode-help" className="field-helper transcription-helper">Standard preserves the stable English flow. Arabic Intelligence keeps provider evidence while adding conservative vocabulary and linguistic observations.</p>{settings.transcription.mode === "arabic_intelligence" && <label className="transcription-option"><input type="checkbox" checked={settings.transcription.preprocessModerateAudio} onChange={(event)=>updateSettings({...settings,transcription:{...settings.transcription,preprocessModerateAudio:event.target.checked}})}/> Request optional moderate-phone-audio preprocessing</label>}<label className="dialect-label" htmlFor="arabic-dialect">ARABIC DIALECT</label><p id="arabic-dialect-help" className="field-helper dialect-helper">Choose a dialect when known. Auto Detect is safest for uncertain calls; mixed Arabic and English keeps automatic text direction.</p><select id="arabic-dialect" aria-describedby="arabic-dialect-help" value={state.metadata.selectedDialect} onChange={(event)=>updateMetadata({selectedDialect:event.target.value as CallMetadata["selectedDialect"]})}>{DIALECTS.map((value)=><option key={value}>{value}</option>)}</select><label>CALL TYPE</label><select value={state.metadata.callType} onChange={(event)=>updateMetadata({callType:event.target.value})}>{CALL_TYPES.map((value)=><option key={value}>{value}</option>)}</select><div className="scorecard-chip"><small>ACTIVE QA SCORECARD</small><b>{scorecard?.name}</b><Link href="/settings">Edit weights</Link></div><div className="privacy"><span>◈</span><p><b>{settings.maskSensitiveInformation ? "Privacy masking is on" : "Privacy masking is off"}</b><br/>{settings.maskSensitiveInformation ? "Sensitive patterns are masked on the server before live analysis." : "The transcript will be sent without masking."}</p></div></div></div>
...
function Practice() {
  ...
  return <section className="content narrow practice-page"><div className="page-intro"><span className="eyebrow accent">INTERACTIVE COACHING</span><h1>Practice Role-play</h1><p>Rehearse difficult conversations with a dialect-aware AI customer, then request evidence-based feedback.</p></div>
    {!active && !turns.length ? <div className="roleplay-grid">
      <div className="panel settings-card roleplay-settings-card"><h2>Scenario setup</h2><FieldSelect label="Scenario type" value={setup.scenarioType} values={["Angry billing customer","Hesitant medical patient","Price-sensitive sales prospect","Customer requesting escalation","Collections payment objection"]} onChange={(value)=>setSetup({...setup,scenarioType:value as RolePlaySetup["scenarioType"]})}/><FieldSelect label="Customer mood" value={setup.customerMood} values={["Concerned","Frustrated","Angry","Hesitant","Skeptical"]} onChange={(value)=>setSetup({...setup,customerMood:value as RolePlaySetup["customerMood"]})}/><FieldSelect label="Difficulty" value={setup.difficulty} values={["Beginner","Intermediate","Advanced"]} onChange={(value)=>setSetup({...setup,difficulty:value as RolePlaySetup["difficulty"]})}/><div className="difficulty-key" role="group" aria-label="Difficulty levels"><button type="button" aria-pressed={setup.difficulty==="Beginner"} className={setup.difficulty==="Beginner"?"active":""} onClick={()=>setSetup({...setup,difficulty:"Beginner"})}>Easy<small>Beginner</small></button><button type="button" aria-pressed={setup.difficulty==="Intermediate"} className={setup.difficulty==="Intermediate"?"active":""} onClick={()=>setSetup({...setup,difficulty:"Intermediate"})}>Intermediate<small>Intermediate</small></button><button type="button" aria-pressed={setup.difficulty==="Advanced"} className={setup.difficulty==="Advanced"?"active":""} onClick={()=>setSetup({...setup,difficulty:"Advanced"})}>Advanced<small>Advanced</small></button></div><FieldSelect label="Language" value={setup.language} values={["English","Arabic"]} onChange={(value)=>setSetup({...setup,language:value as RolePlaySetup["language"]})}/><FieldSelect label="Arabic dialect" value={setup.dialect} values={DIALECTS} onChange={(value)=>setSetup({...setup,dialect:value as RolePlaySetup["dialect"]})}/><label className="field-label" htmlFor="coaching-goal">COACHING GOAL</label><textarea id="coaching-goal" className="goal-input" value={setup.coachingGoal} onChange={(event)=>setSetup({...setup,coachingGoal:event.target.value})}/></div>
      <div className="panel roleplay-ready"><span className="eyebrow accent">WORKFLOW PREVIEW · STATIC EXAMPLE</span><h2>See the coaching loop before you connect live AI</h2><p>This preview explains the interaction only. It is not a running simulation and does not produce a result.</p><div className="roleplay-preview" role="list" aria-label="Workflow preview sequence"><article className="customer" role="listitem"><small>AI CUSTOMER · PREVIEW</small><p>“I’ve already called twice. Why should I trust this will be fixed?”</p></article><article className="agent" role="listitem"><small>AGENT RESPONSE · PREVIEW</small><p>“I understand. I’ll check it now.”</p></article><article className="coach" role="listitem"><small>AI COACH FEEDBACK · PREVIEW</small><p>Acknowledge the repeat effort, then give a specific owner and timeframe.</p></article></div><button className="primary" onClick={start} disabled={busy}>{busy?"Connecting…":"Start Practice"}</button><small className="roleplay-key-note">Live AI required · no reply is simulated without a configured server key.</small></div>
...
function FieldSelect({label,value,values,onChange}:{label:string;value:string;values:readonly string[];onChange:(value:string)=>void}) { const id=`roleplay-${label.toLowerCase().replaceAll(" ","-")}`; return <div className="setting-row"><label htmlFor={id}>{label}</label><select id={id} value={value} onChange={(event)=>onChange(event.target.value)}>{values.map((item)=><option key={item}>{item}</option>)}</select></div>; }
