...
function Results({ example }: { example: boolean }) {
  ...
  return <section className="content results-page">
    ...
    <div className="results-head"><div><span className="eyebrow accent">{example ? "SAMPLE ANALYSIS" : "VALIDATED LIVE ANALYSIS"}</span><h1>{analysis.executiveSummary.split(".")[0]}.</h1><p>{state.metadata.agentName} · {state.metadata.callType} · {analysis.detectedLanguage} · Confidence {analysis.confidenceScore}%</p></div><div className="result-actions"><Link href={reportHref} className="secondary">↓ Download Summary</Link><div className="score-ring" style={{"--score":`${analysis.overallScore*3.6}deg`} as React.CSSProperties}><div><b>{analysis.overallScore}</b><small>OVERALL</small></div></div></div></div>
...
function Practice() {
  ...
  return <section className="content narrow"><div className="page-intro"><span className="eyebrow accent">INTERACTIVE COACHING</span><h1>Practice Role-play</h1><p>Rehearse difficult conversations with a dialect-aware AI customer, then request evidence-based feedback.</p></div>
    {!active && !turns.length ? <div className="roleplay-grid"><div className="panel settings-card"><h2>Scenario setup</h2><FieldSelect label="Scenario type" value={setup.scenarioType} values={["Angry billing customer","Hesitant medical patient","Price-sensitive sales prospect","Customer requesting escalation","Collections payment objection"]} onChange={(value)=>setSetup({...setup,scenarioType:value as RolePlaySetup["scenarioType"]})}/><FieldSelect label="Customer mood" value={setup.customerMood} values={["Concerned","Frustrated","Angry","Hesitant","Skeptical"]} onChange={(value)=>setSetup({...setup,customerMood:value as RolePlaySetup["customerMood"]})}/><FieldSelect label="Difficulty" value={setup.difficulty} values={["Beginner","Intermediate","Advanced"]} onChange={(value)=>setSetup({...setup,difficulty:value as RolePlaySetup["difficulty"]})}/><FieldSelect label="Language" value={setup.language} values={["English","Arabic"]} onChange={(value)=>setSetup({...setup,language:value as RolePlaySetup["language"]})}/><FieldSelect label="Arabic dialect" value={setup.dialect} values={DIALECTS} onChange={(value)=>setSetup({...setup,dialect:value as RolePlaySetup["dialect"]})}/><label className="field-label">COACHING GOAL</label><textarea className="goal-input" value={setup.coachingGoal} onChange={(event)=>setSetup({...setup,coachingGoal:event.target.value})}/></div><div className="panel roleplay-ready"><span>◌</span><h2>Ready when live AI is</h2><p>The full workflow renders without a key. Start Practice will clearly report that live AI is not configured and will never simulate a customer reply.</p><button className="primary" onClick={start} disabled={busy}>{busy?"Connecting…":"Start Practice"}</button></div></div> : <div className="practice-session"><div className="panel chat-panel"><div className="panel-head"><div><span className="eyebrow">{setup.scenarioType.toUpperCase()}</span><h2>{setup.customerMood} customer · {setup.difficulty}</h2></div><span className="live-pill"><i/> Live role-play</span></div><div className="chat-messages" dir={setup.language==="Arabic"?"rtl":"ltr"}>{turns.map((turn,index)=><div className={`chat-message ${turn.role}`} key={`${turn.role}-${index}`}><small>{turn.role==="customer"?"AI CUSTOMER":"YOU · AGENT"}</small><p>{turn.content}</p></div>)}</div>{active&&<div className="chat-composer"><textarea value={draft} onChange={(event)=>setDraft(event.target.value)} placeholder="Type the agent response…" dir={setup.language==="Arabic"?"rtl":"ltr"}/><button className="primary" onClick={send} disabled={busy||!draft.trim()}>{busy?"Waiting…":"Send response"}</button></div>}<div className="session-actions"><button className="secondary" onClick={()=>callRolePlay("feedback")} disabled={busy||!turns.length}>Request feedback</button><button className="danger-button" onClick={()=>callRolePlay("end")} disabled={busy||!turns.length}>End practice</button></div></div>{feedback&&<RolePlayFeedback feedback={feedback}/>}</div>}
